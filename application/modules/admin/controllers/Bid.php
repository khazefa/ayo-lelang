...
class Bid extends Back_Controller
{
	...
	public function index()
	{
		...
		foreach ($rs_bid as $rb) {
			...
			$row['item_id'] = empty($rs_produk) ? 0 : $rs_produk[0]['id_lelang'];
			$row['item_img'] = empty($rs_produk) ? "undefined.jpg" : $rs_produk[0]['gambar_produk'];
			$row['item_name'] = empty($rs_produk) ? "Item telah dihapus" : $rs_produk[0]['nama_lelang'];
			$row['bidder_name'] = $rs_peserta[0]['nama_peserta'];
			$row['max_price'] = empty($rs_produk) ? 0 : $rs_produk[0]['harga_maksimal'];
	...
	public function orders()
	{
		...
		foreach ($rs_order as $rb) {
			...
			$idpelelang = empty($rs_items) ? 0 : $rs_items[0]['id_pelelang'];
			$rs_auctioner = $this->MPelelang->get_data_info2($idpelelang);
			...
			$row['auctioner'] = empty($rs_auctioner) ? "Undefined" : $rs_auctioner[0]['nama_pelelang'];
